...
static PGELogicHandler *s_logic_handler = NULL;
static PGERenderHandler *s_render_handler = NULL;
...
Window* pge_begin(PGELogicHandler *logic_handler, PGERenderHandler *render_handler, PGEClickHandler *click_handler) {
  ...
  // Go!
  window_stack_push(s_game_window, false);
...
GSize get_screen_size() {
  return layer_get_bounds(s_canvas).size;
}
